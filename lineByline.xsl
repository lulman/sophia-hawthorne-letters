 ...
 <!--  <xsl:template match="body">
   ...
   <xsl:template match="pb">
      <div class="pagebreak">
        ...
        (<a><xsl:attribute name="HREF">http://people.cohums.ohio-state.edu/ulman1/Sophia_Hawthorne_Letters/zoom.cfm?file=<xsl:value-of select="@id"/>.jpg</xsl:attribute><xsl:attribute name="target">top</xsl:attribute>click to open page image in a new window</a>)]<br/>
      </div>
      <!-- Removed by -->
      <!-- <HR width="90%"/> -->
   </xsl:template>
   <xsl:template match="lb">
      <br/>
      <img><xsl:attribute
         name="src">http://ts1.cohums.ohio-state.edu/images/people\ulman1\SHawthorne_Letters/<xsl:value-of
               select="preceding::pb[1]/@id"/>.jpg?RGN=<xsl:value-of
               select="@coord"/>&amp;cell=800,300&amp;cvt=jpeg</xsl:attribute></img>
